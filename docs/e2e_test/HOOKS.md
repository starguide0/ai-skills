...
> 최종 업데이트: 2026-03-09 | Test Skill v6.6
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/훅이름.py\"",
            "timeout": 10
          }
        ]
      }
...
| `"Edit"`                          | Edit 도구 호출 시          |
| `"Bash"`                          | 모든 Bash 명령 실행        |
| `"mcp__postgres_service-out__query"` | 해당 MCP 도구 호출 시      |
...
**대상**: `_test_sheet_` 포함 파일

**목적**: 테스트시트가 표준 섹션 구조와 TC 형식을 준수하는지 JSON Schema(`_test_sheet_schema.json`)와 연동하여 강제
...
**대상**: `_data_mapping.json` 포함 파일

**목적**: 데이터 매핑 파일의 스키마 유효성과 수치 정합성을 JSON Schema(`_data_mapping_schema.json`) 기반으로 강제
...
TC-N.json 저장 시도
        ↓
partial_results/ 부모 디렉터리에서 *_data_mapping.json 탐색
        ↓
...
python3 behavioral_gate.py \
  --mapping ARG-XXXXX_data_mapping.json \
...
```json
{
  ...
  "results": [
    ...
    },
    {
      "tc_id": "TC-2.1",
      "gate": "BLOCKED",
      "reason": "behavioral_check.verdict=FAIL: 상태 불일치"
    }
  ],
...
- MCP PostgreSQL 도구 후 → 5개 DB 전체 (service-out/service-in/inventory/job/metadata)
...
[Caution Remind] 관련 주의사항 (cross_service, join, mcp_postgres, null, query):
  - 패턴 1: INNER JOIN 누락 시 데이터 누락
  - 패턴 5: 상태(state) 컬럼 비교 시 ENUM 타입 주의
상세 내용: $CLAUDE_PLUGIN_ROOT/skills/e2e_test/_rules/_caution_common_errors.md 참조
```
...
```json
{
  ...
  "decisions": [{ "decision": "판단 내용", "reason": "이유" }],
  "verification": {
    "method": "어떻게 검증했는가",
...
PostToolUse:
  ...
  mcp__postgres_service-out__query → remind_caution.py       (5s)  ← DB 쿼리 후 리마인드
  mcp__postgres_service-in__query  → remind_caution.py       (5s)
...
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/WO-99_test_sheet_v1.md","content":"# empty file"}}' | \
  python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/validate_test_sheet.py

# validate_data_mapping.py 테스트 — 필수 필드 누락으로 deny 발생 확인
echo '{"tool_name":"Write","tool_input":{"file_path":"/tmp/WO-99_data_mapping.json","content":"{}"}}' | \
  python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/validate_data_mapping.py

# remind_caution.py 테스트 — MCP 도구 시뮬레이션
echo '{"tool_name":"mcp__postgres_service-out__query","tool_input":{"sql":"SELECT a.id, b.name FROM service-out_order a JOIN service-out_batch b ON a.id = b.id WHERE a.state IS NULL"}}' | \
  python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/remind_caution.py
```
...
python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/behavioral_gate.py \
  --mapping test/WO-55_worker_assignment/WO-55_data_mapping.json
```
...
1. `$CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/새_훅.py` 작성:
...
2. `.claude/settings.local.json`에 등록:
   ```json
   {
     ...
     "hooks": [
       {
         "type": "command",
         "command": "python3 \"$CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/새_훅.py\"",
         "timeout": 10
       }
     ]
   }
...
`$CLAUDE_PLUGIN_ROOT/skills/e2e_test/_rules/_caution_common_errors.md`에 키워드 태그 추가:
