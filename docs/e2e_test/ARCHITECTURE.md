...
| **Issue-First**          | 테스트의 첫 단계는 항상 이슈(IMS) vs 구현 비교 | `test-gate`가 이슈를 먼저 조회하고, 구현과 비교   |
...
| **Traceability**        | 모든 판단 근거가 파일로 남음               | 테스트시트에 issue_digest + code_digest 저장      |
| **Extensibility**       | 프로젝트별 후처리를 선언적으로 확장        | `test/_post.md`로 보고서 발행/이슈 연동           |
...
이로 인해 Gate 2회 실행, 이슈(IMS) 이중 조회, Report 이중 생성 문제가 발생했습니다.
이를 `test-run` 하나로 통합하여 모든 중복을 제거했습니다.

**왜 Issue-First인가?**

코드만 분석하면 "코드가 하는 대로"가 정답이 됩니다. 코드에 버그가 있어도 "정상"으로 판정됩니다.
이슈(IMS) 요구사항을 먼저 확인하고, 구현과 비교해야 진짜 버그를 잡을 수 있습니다.
...
행위적 조건은 `ctx.service_metadata`가 제공하는 semanticContracts/dataFlowPaths를
활용하여 코드의 런타임 진입 조건을 데이터 매핑 필터로 전달합니다.

**왜 test_baseline을 시트에 저장하는가?**

다음 테스트 실행 시 "무엇이 변경되었는가"를 자동 감지하기 위함입니다.
Gate가 현재 이슈(IMS)/Code와 시트에 저장된 issue_digest/code_digest를 비교하여
Gate가 현재 이슈(IMS)/Code와 시트에 저장된 issue_digest/code_digest를 비교하여
REUSE(재사용) / REPLAN(재생성) / NEW(신규) 판정을 자동으로 수행합니다.

**왜 CLI-First 및 playwright_utils 추출인가? (v6.8)**

MCP 서버 레이어의 오버헤드를 제거하여 UI 테스트 성능을 극대화하고, Playwright 관리 기능을 독립 스킬로 격리하여 여러 스킬에서 재사용 가능한 브라우저 제어 기반을 마련했습니다.
...
graph TD
    ...
    subgraph "Step 1: Verify"
        ...
        RefData --> Gate["test-gate<br>이슈 vs 구현 비교"]
        Gate --> CompareReport[불일치 리포트]
        CompareReport --> Q1{Q1: 진행?}
        Q1 -- No --> Stop([종료])
        Q1 -- Yes --> Q2{Q2: 테스트 기준?}
        Q2 --> |Issue / Code / Union| Baseline[test_baseline 확정]
    end
    ...
    subgraph "Step 3: Plan"
        ...
        Plan --> Sheet[테스트시트<br>issue_digest + code_digest 포함]
    end
    ...
    subgraph "Step 7: Post (선택적)"
        ...
        Post --> Confirm{사용자 확인}
        Confirm -- Yes --> Actions["보고서 발행<br>이슈 코멘트 등"]
        PostChk -- No --> Skip7([스킵])
    end
...
| **test-gate**         | v4.2 | Step 1    | 이슈 vs 구현 비교 + 서비스 탐색 + 서버 접속 확인 | 가능      | 티켓 ID, 브랜치          | test_baseline         |
| **test-plan**         | v2.3 | Step 3    | 영향도 분석, 시트 생성, 행위적 조건 도출         | 가능      | test_baseline            | 테스트시트.md         |
| **test-data**         | v2.3 | Step 4    | TC별 데이터 매핑, Purpose Fitness Check          | 가능      | test_sheet               | data_mapping.json       |
...
| **test-reporter**     | v4.1 | Step 6    | 결과 병합, 보고서 생성                           | 가능      | partial_results/         | 테스트결과 보고서     |
| **test-review-agent** | v1.0 | Step 6.5  | 보고서 의미적 검증 (Worker)                      | 가능      | 보고서 + 시트 + baseline | {ticket}_review.json |
| **test-post**         | v1.1 | Step 7    | 후처리 액션 (보고서/이슈)                         | 가능      | ctx + _post.md          | 액션 실행 결과        |
...
| **test/\_post.md**                | 프로젝트별 후처리 액션 정의 (보고서 발행, 이슈 코멘트 등) |
...
┌──────────────────────────────────────────────────────┐
│           IMS / Wiki / Browser (외부 시스템)          │
│  postgres_*  → DB 조회 (Read-only)                    │
│  ims_provider → 이슈(IMS), ClickUp 등                     │
│  wiki_provider → 보고서, Ocean Wiki 등           │
...
LLM (Main)
  ...
  │     validate_report_structure.py  — 보고서 문서 → 체크리스트
  │
...
### 3.4 Hybrid Execution Model

1.  **테스트 스크립트 (UI/API)**
    - UI: `playwright` (빠른 실행 및 회귀 테스트용) + `agent-browser` (초기 스크립트 작성 및 빠른 DOM 분석, 에러 복구용)
    - API: `axios` 또는 내장 모듈
    - DB: 직접 쿼리로 사전/사후 상태 검증
    - Event: Kafka/RabbitMQ 메시지 발행 및 소비 (필요시)
2.  **LLM 추론 최소화 (Determinism 제고)**
    - 파이프라인 간 상태 전달 시 JSON 기반 컨텍스트 명시.
    - **Git Hash 기반 캐싱**: `git rev-parse HEAD`, `git diff HEAD`, `git ls-files --others --exclude-standard` (Untracked 파일) 결과 및 환경변수 설정 파일 스냅샷을 묶어 완전한 Git State Hash를 계산합니다. `frontend`, `backend` 디렉토리의 Hash(FE_STATE, BE_STATE) 변경 여부를 확인하여, 코드가 변경되지 않았다면 기존에 생성된 UI 스크립트나 데이터 매핑 파일(Asset)을 **무조건 재사용**합니다. LLM 개입 없는 판단으로 캐시 히트율과 실행 속도를 극대화합니다.
3.  **Fail-Fast & Tolerance**
    - 설정된 N/T (Not Tested) 임계치(`MAX_NT_TOLERANCE`)를 초과하거나, Git Hash 불일치 등으로 필수 Asset 확보에 실패하면 파이프라인을 즉시 중단합니다.
...
### 3.6 상태 관리 — Write-Through + Read-Through Cache
...
| sheet           | `{ticket}_test_sheet_v*.md`            | Glob → 최고 v{N} 선택                                                         |
| data_mapping    | `{ticket}_data_mapping.json`           | Read → JSON parse                                                             |
| execution_plan  | `{ticket}_execution_plan.json`         | Read → JSON parse                                                             |
| partial_results | `partial_results/*.json`               | Glob → 경로 목록                                                              |
| report_file     | `{ticket}_report_test_result_v*.md`    | Glob → 최신 버전                                                             |
...
| auth_url        | `test/_shared/env/api_endpoints.md`  | Read → AUTH_URL 추출                                                          |

> **ctx.ticket, ctx.ticket_folder**: 파일에서 복원 불가. 유실 시 사용자에게 재질문.
> **ctx.auth_body**: 파일 저장 금지 (보안). 컨텍스트 압축 후에는 `test/_shared/env/accounts.md`에서 loginId/password 재추출 후 json.dumps()로 재생성.
...
### 3.7 Artifact Garbage Collection (산출물 가비지 컬렉션)

파이프라인 실행 시 기존의 임시 산출물이 현재 실행을 오염시키는 것을 방지하기 위해 자동 가비지 컬렉션을 수행합니다.

- **실행 시점**: `test-run` 오케스트레이터 시작 단계 및 시트의 `REPLAN`, `NEW` 판정 시.
- **정리 대상**: 이전 테스트 실행의 잔여물 (`partial_results/*.json`, `_execution_plan.json`, `_review.json` 등).
  - **[전체 재시작]**: `partial_results/` 폴더를 강제 삭제하여 이전 세션 오염 방지.
  - **[이어서 진행]**: `partial_results/`를 유지하여 기존 성공 TC 결과를 재활용 (Resume).
  - **[성공 종료]**: 파이프라인 마지막 단계에서 `partial_results/`를 최종 삭제 (Cleanup).
- **효과**: 이전 실행의 실패 파편이 새 결과 병합 단계(Step 6)에 섞여 들어가는 심각한 오류(False-Positive/Negative)를 구조적으로 원천 차단합니다.

### 3.8 Pipeline State Machine (테스트 상태 전이 모델)

파이프라인은 단순 선형 실행이 아닌, 명확한 State Machine(상태 기계) 기반으로 동작합니다.

- **상태 전이 통제**: `test-run` 오케스트레이터는 하위 스킬(`test-gate`, `test-plan` 등)의 명시적 반환값(상태 코드)에 따라 조건부 분기를 결정합니다.
- **상태 영속성**: 실행 도중 중단되더라도 파일 시스템 기반의 캐시(Test Baseline 등)를 검사하여 마지막으로 수행된 단계부터 자연스럽게 재개(Resume)가 가능합니다.
- **Context Lifecycle**: Step 전환 시 필수적으로 컨텍스트를 소거(GC)하고 다음 Step에 필요한 최소 메타데이터만 다시 로드하여 토큰 한계(Context Window)를 효과적으로 방어합니다.
...
data_mapping.json update: Tier 완료 후 순차 처리.
...
| 외부 테스트 프레임워크 (pytest) | 동적 테스트 계획, 이슈(IMS) 연동에 부적합              |
...
│   │   └── _report_format_rules.md
│   │
│   └── {ticket}_{feature_name}/                   ← Per-feature output folder
│       ├── {ticket}_test_sheet_v{N}_{date}.md
│       ├── {ticket}_data_mapping.json             ← [SSOT] Structure signatures + sample file links
│       ├── {ticket}_report_test_result_v{N.M}_{date}.md
│       ├── samples/                          ← 입력 샘플 (Excel 등) 누적 폴더
│       └── partial_results/                  ← [TEMP] 실행 상태 (Restart 시 초기화)
│           └── *.json
│
├── .claude/
│   └── skills/e2e_test/                       ← 스킬 정의
│       ├── SKILL.md                       ← 진입점
│       ├── README.md                      ← 사용 가이드
│       ├── doc/                           ← 설계 문서
...
│
└── service metadata provider            ← 참조 메타데이터 (file/cli/rag/mcp 중 하나 이상)
    ├── services.json
...
| 구분           | `$CLAUDE_PLUGIN_ROOT/skills/e2e_test/_rules/` | `test/_shared/rule/`          |
| -------------- | ----------------------------------------------------- | ----------------------------- |
| **한 줄 요약** | "어떻게 테스트해야 하는가"                            | "이 프로젝트에서 조심할 것들" |
| **소유자**     | 스킬 개발자                                           | 테스트 팀 (YOUR_PROJECT)      |
| **이식성**     | 다른 프로젝트 그대로 사용 가능                        | YOUR_PROJECT에만 의미 있음    |
| **생명주기**   | 스킬 버전과 함께 관리                                 | 실수 발생 시 축적             |
| **비유**       | 교과서                                                | 현장 메모장                   |

---

## 4.6 JSON Schema Integration (스키마 기반 검증)

`$CLAUDE_PLUGIN_ROOT/skills/e2e_test/_rules/` 내의 주요 검증 로직은 하드코딩된 Python 로직에서 **JSON Schema**(`_data_mapping_schema.json`, `_test_sheet_schema.json` 등)를 참조하는 동적 파서 구조로 통합되었습니다.

- **파서 순수성 분리**: Python 훅 스크립트(`validate_*.py`)는 스키마를 로드하고 유효성을 검사하는 순수 실행기(Parser) 역할만 수행합니다.
- **선언적 규칙 관리**: 룰의 변경(조건 추가나 필수 필드 변경)은 파이썬 코드 수정 없이 확장 가능한 설계(Decoupled Validation)를 보장합니다.
...
Step 1 (Gate v4.1)
  ...
  → ctx.test_baseline    = { mode, baseline_mode, issue_digest, code_digest, comparison, test_scope }
  ...
  "issue_digest": [
    {
      "id": "J1",
      "content": "요구사항 내용",
      "source": "description | acceptance_criteria | subtask"
    }
  ],
  "code_digest": [
...
```json
{
  ...
  "structural_signatures": {
    // [NEW] 샘플 데이터 재사용을 위한 구조 판정 기준
    "excel_v1": {
      "type": "xlsx",
      "sheets": ["Master", "Detail"],
      "headers": { "Master": ["ID", "Name"], "Detail": ["Ref_ID", "QTY"] }
    }
  },
  "mappings": {
    "TC-1.1": {
      "status": "MAPPED | NOT_FOUND | PROVISIONING_NEEDED | PROVISIONED",
      "sample_file": "samples/data_v1.xlsx", // [NEW] 시그니처와 매칭된 실제 파일
      "data": { "entity_code": "ENT-00001", "partition_id": 1001 },
      "behavioral_check": {
        "verdict": "PASS",
        "conditions": ["service-out_order.state = READY"],
        "method": "DB 직접 조회"
      }
    }
  },
...
| `repo-registry.json` 스캐폴딩   | 프로젝트 서비스 목록 및 Git URL 관리 파일 생성 |
| Scaffold files (env, accounts, api) | Create template with `{TODO}` → Request user input |
...
1.  **사람이 판단한다** — 이슈(IMS)와 구현이 다를 때 Gate는 판단 재료만 제공
...
```
참조 데이터 확인 → 이슈(IMS) 수집 → 코드 분석 → 비교 리포트 → 사용자 의사결정
→ 서비스 탐색 (Remote Reconnaissance) → 범위 판단 → 서버 접속 확인 → baseline 확정
```

#### 1.1 서비스 탐색 및 원격 정찰 (Remote Reconnaissance)

로컬 작업 영역에 소스 코드가 없는 서비스에 대해 `repo-registry.json`에 정의된 Git URL을 사용하여 원격으로 정찰을 수행합니다.

1.  **Local Discovery**: 워킹 디렉토리 내의 하위 폴더를 스캔하여 `.git` 구성을 확인합니다.
2.  **Remote Discovery**: 로컬에 없는 서비스는 Registry의 URL을 통해 원격 브랜치 및 커밋 이력을 조회합니다.
3.  **Validation Routine**: 발견된 모든 서비스에 대해 실제 접근 가능 여부를 검증합니다.
4.  **Error Handling (Skip/Abort)**: 접근 실패 시 사용자에게 `[Skip]`(해당 서비스 제외) 또는 `[Abort]`(파이프라인 중단)를 제안하여 실행 흐름을 제어합니다.

---

### Step 2: Ticket Namespace & Folder Resolution

이 스킬은 데이터 일관성을 위해 **"1 Ticket = 1 Folder"** 원칙을 시스템적으로 강제합니다.

#### 2.1 정밀 티켓 ID 추출 (Refined Regex)

문자열 내에서 다른 텍스트와 혼동되지 않도록 정밀한 정규표현식을 사용하여 티켓 ID를 추출합니다.

- **Regex**: `([A-Z]+-\d+)(?!\d)`
- **효과**: `ABC-1234A`와 같은 패턴에서 뒤의 `A`를 제외하고 순수 티켓 ID인 `ABC-1234`만 정확하게 매칭합니다.

#### 2.2 폴더 결정 알고리즘 (RESOLVE)

1.  **ID 기반 검색**: 작업 폴더(`test/`) 내에서 추출된 티켓 ID로 시작하는 폴더를 검색합니다.
2.  **재사용 (Reuse)**: 기능 설명(Feature Description) 부분이 다르더라도 티켓 ID 부분이 일치하는 기존 폴더가 있다면 이를 우선적으로 사용합니다.
3.  **신규 생성**: 기존 폴더가 없는 경우에만 `{Ticket_ID}_{Feature_Description}` 형식으로 새 폴더를 생성합니다.
...
| issue_digest, code_digest | 이슈(IMS)/Code 정규화 항목                                                    |
...
### Step 2: Sheet Check — 시트 버전 자동 판정 (캐싱 메커니즘 1단계)
...
1. **이슈(IMS)/Code Digest**: 기존 시트의 `Section 0`에 저장된 `issue_digest` + `code_digest`와 현재 Gate 결과를 **의미적으로** 비교.
2. **Git State Hash (신규)**: `frontend`와 `backend` 파트의 현재 상태(`git rev-parse HEAD` + `git diff HEAD` + `git ls-files --others --exclude-standard` + 환경변수 스냅샷)를 추출하여, 이전에 저장된 `_state.json` 상의 `FE_STATE`, `BE_STATE` 해시 값과 완전 동일한지(Strict Match) 확인.

위의 조건들 중 하나라도 변경이 감지되면 REPLAN으로 판정하여 캐시를 무효화합니다.

---

### Step 3: Plan — 테스트시트 생성 (`test-plan`)

- 이슈(IMS)를 **재조회하지 않음** (Gate 결과 사용)
- 시트 Section 0에 `issue_digest` + `code_digest` 저장 (다음 Gate 비교용)
...
```
✅ 올바른 흐름:  이슈(IMS) 요구사항 → 기대결과 도출 → 코드 분석으로 검증 방법 결정
❌ 잘못된 흐름:  코드 분석 → 코드 동작을 기대결과로 설정 → 이슈(IMS) 무시
```
...
#### 5.2 Execution (Tiered Loop) & 스크립트 도출

```text
FOR each Tier:
  - 현재 Tier의 parallel_tasks 동시 실행
  - PROVISION → test-provisioning Worker 호출
  - TEST      → 실제 테스트 로직 (API 호출 + DB 검증 + UI)
    - UI Test의 경우:
      1. Git Hash 검증 통과 + 기존 스크립트 존재 → Asset 재사용 (Fast Path)
      2. 구조 변경/신규 작성 필요 시 → `playwright_utils` 스킬을 통해 `agent-browser` 또는 Playwright CLI 호출하여 DOM/상태 파악
      3. Playwright 스크립트 실행 및 결과 수집 (Asset 저장)
      4. `_state.json` 갱신 (FE_STATE, BE_STATE 명시적 저장)
...
`partial_results/*.json`을 수집하여 단일 보고서 문서로 병합.
...
| 액션                | 설명                            |
| ------------------- | ------------------------------- |
| `confluence-upload` | 보고서 페이지 생성/업데이트 |
| `jira-comment`      | 이슈(IMS) 티켓에 코멘트 추가         |
...
**사용 가능한 변수:** `{ticket}`, `{summary}`, `{version}`, `{date}`, `{result}`, `{pass_count}`, `{fail_count}`, `{total_count}`, `{branch}`, `{report_file}`, `{sheet_file}`
...
| 버전 | 날짜       | 변경 내용                                                                                          |
| ---- | ---------- | -------------------------------------------------------------------------------------------------- |
| v6.6 | 2026-03-09 | Git State 기반 캐싱(로컬 변경 포함) 및 Vercel `agent-browser` 통합 내용 반영, 문서 익명성 강화     |
| v6.5 | 2026-02-25 | JSON Schema 기반 검증 병합, Pipeline State Machine 확립, Artifact Garbage Collection 도입          |
