...
> **목표**: 이번 분석에서 LLM 추론으로 어렵게 발견한 버그를 다음 실행에서는 bash/grep으로 기계적으로 잡을 수 있도록 analyze.md에 변환·저장한다.
> Phase 5 완료 후 실행한다. **사용자 확인 없이 파일을 수정하지 않는다.**

> **에이전트 등급**: `Crystallizer` ( **$GRADE_C** )
> - `facts.model_map.GRADE_C` 변수에 할당된 모델을 사용하여 호출하십시오.
