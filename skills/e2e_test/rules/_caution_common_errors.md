...
- **컨텍스트 압축 후 복원**: `auth_url` → `test/_shared/env/api_endpoints.md`, `auth_body` → `test/_shared/env/accounts.md`에서 재구성
...
- **스키마 prefix 필수**: 테이블명에 스키마 prefix 필요 (예: `public.service-out_order`, `core-system.container_lifecycle`)
- **cross-DB 참조 불가**: 각 MCP는 단일 DB에만 연결. service-out-service 쿼리에서 inventory-service 테이블 직접 JOIN 불가
...
<!-- keywords: cross_service, service-out, service-in, inventory, job, metadata -->
## 크로스 서비스 DB 쿼리 주의사항

- **서비스별 MCP 분리**: service-out-service(`mcp__postgres_service-out__query`), service-in-service(`mcp__postgres_service-in__query`), inventory-service(`mcp__postgres_inventory__query`), task-service(`mcp__postgres_job__query`), meta-service(`mcp__postgres_metadata__query`)
- **ID 의미 확인**: 동일한 ID가 서비스마다 다른 의미를 가질 수 있음 (`ctx.service_metadata`의 data contracts 참조)
