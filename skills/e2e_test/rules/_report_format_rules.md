...
```
1. Read: test/templates/보고서_test_result_template.md (필수 — 섹션 구조)
...
### Step 6-1: 보고대상 테스트결과 문서 생성 (새 파일)

- 파일명: `{ticket}_report_test_result_v{N}.{M}_{YYYY-MM-DD}.md`
...
[ ] 테스트 정보: Env/Auth/DB/Workplace/Date-Time/User/Verification/Sheet-Ver/Report-Ver
```
...
> 3. **`<br>` 사용 금지** — 보고대상에서 리터럴 텍스트로 표시됨. 대신 마크다운 줄바꿈 사용
...
> 13. **Mermaid 코드블록 식별자 필수** — 마크다운 문서 내에 Mermaid 코드를 작성할 때는 반드시 ` ```mermaid ` 처럼 언어 식별자를 명시해야 한다. 식별자 없는 일반 ` ``` ` 코드블록 사용 엄격히 금지.

---

## 근거 작성 형식

**템플릿 참조**: `test/templates/보고서_test_result_template.md` 를 1:1 대응시킨다. 임의로 섹션을 생략하거나 형식을 변경하지 않는다.
...
테스트결과와 보고대상 문서에 Mermaid 다이어그램을 포함하여 이해도를 높인다.
...
### 보고대상용 Mermaid → mermaid.live URL 변환 (필수!)

> **보고대상는 Mermaid를 네이티브 렌더링할 수 없다.**
> 보고대상 문서에서는 Mermaid 코드블록 대신 **mermaid.live/edit 링크**를 생성한다.
...
```bash
python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/generate_mermaid_urls.py <<'MERMAID_INPUT'
{
...
| 항목 | 테스트결과 파일 | 보고대상 파일 |
|------|---------------|----------------|
| Mermaid 표기 | 원본 코드블록 (반드시 ` ```mermaid ` 식별자 사용) | **mermaid.live URL 링크** |
| 링크 텍스트 | N/A | `[{다이어그램 제목} ({유형})]({URL})` |
| 코드블록 유지 | ✅ (일반 ` ``` ` 사용 불가) | ❌ (링크로 대체) |
...
`{ctx.ticket_folder}/{ticket}_report_test_result_v{N}.{M}_{YYYY-MM-DD}.md`
...
| Tester | Claude |
| Verification Method | MCP PostgreSQL DB 쿼리 + API |
| Test Sheet Version | v{N} |
| Test Result Version | v{N}.{M} |
```
