---
name: test-plan
description: "Generates the test sheet and tc_spec.json based on Gate's output (Issue-First approach)."
---
...
- `test_concerns.md` — DB schema errors, recurring mistakes prevention (Loaded first)
- `domain/{related_domain}.md` — Domain business knowledge for testing
...
| 필드                                   | 출처              | 필수 | 설명                                                                                  |
| -------------------------------------- | ----------------- | ---- | ------------------------------------------------------------------------------------- |
| ctx.test_baseline.issue_digest         | test-gate         | Y    | 정규화된 이슈 요구사항 (test-run 경유 시)                                             |
| ctx.test_baseline.code_digest          | test-gate         | Y    | 정규화된 구현 사항 (test-run 경유 시)                                                 |
| ctx.test_baseline.comparison           | test-gate         | Y    | 비교 결과 (matched, issue_only, code_only)                                             |
| ctx.test_baseline.baseline_mode        | test-gate         | Y    | 테스트 기준 (match/issue/code/union)                                                   |
| ctx.test_baseline.test_scope           | test-gate         | Y    | 테스트 범위 항목 목록                                                                 |
| ctx.test_baseline.affected_services    | test-gate         | Y    | 탐색된 서비스/노드 목록 (repo, type, device_type, protocol, branch)                    |
| ctx.test_baseline.test_types           | test-gate         | Y    | api_test, db_verify, node_simulation, event_verify, web_ui_capture                   |
...
| 필드                     | 소비자                                   | 설명                                                                                       |
| ------------------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------ |
| Test Sheet File          | test-data, test-scheduler, test-reporter | {ticket}_test_sheet_v{N}_{date}.md                                                        |
| ctx.sheet                | test-data, test-scheduler, test-reporter | Absolute path to test sheet                                                                |
| ctx.sheet_version        | test-data, test-run                      | v{N} — Major version (Increased on TC change)                                              |
| Section 0: Test Baseline | 다음 test-gate (1.8)                     | issue_digest + code_digest + comparison + affected_services + test_types 저장 (변경 감지용) |
| Section 1: 변경 요약     | test-reporter                            | 이슈(IMS) 근거 + 핵심 변경 사항                                                                 |
...
| {ticket}_tc_spec.json   | test-data, test-scheduler                | TC-specific behavioral conditions, expectations, data reqs structured JSON (No NL)         |
| ctx.tc_spec              | test-data, test-scheduler, **test-run**  | Absolute path to tc_spec.json                                                              |
...
> `ctx.test_baseline` missing → Glob `{ctx.ticket_folder}/{ticket}_gate_*.json` → Latest timestamp file Read → JSON parse → `ctx.test_baseline` restoration.
> 파일도 없으면 → 단독 호출 모드로 전환 (Step 1 자체 이슈 조회 실행).

```
수신 데이터:
- ctx.test_baseline.issue_digest        → 이슈 요구사항 (이미 정규화됨)
- ctx.test_baseline.code_digest        → 소스 구현 사항 (이미 분석됨)
- ctx.test_baseline.comparison         → 비교 결과 (matched, issue_only, code_only)
- ctx.test_baseline.baseline_mode      → 테스트 기준 (issue / code / union / match)
...
TC 작성 시 affected_nodes와 test_types 활용 (Universal E2E):
- affected_nodes의 각 노드는 `device_type`(Server/Browser/Mobile/Robot)과 `protocol`(HTTP/WebDriver/gRPC)을 가짐
- Multi-step Stimulus: TC는 여러 노드에 걸친 순차적 액션(Stimulus Sequence)을 가질 수 있음
- Connectivity: `ctx.test_baseline.connectivity_status`를 통해 노드간 연동 확인 여부 참고
- **통합 테스트 (Integrated Test)**: `ctx.test_baseline.integrated_test`가 true인 경우, UI 자극(Stimulus)과 Backend 결과 검증(API/DB)이 결합된 **High-Priority 통합 TC를 반드시 1개 이상 포함**해야 함.
- flow_visualizer.py: TC 설계 단계에서 Mermaid 흐름도를 생성하여 연동 구조 시각화 (Step 4.7 참조)

단독 호출 시 (test-run 경유 없이):
- test_baseline이 없으면 자체적으로 이슈 조회 + 코드 분석 수행
- 이 경우 기존 Step 1(이슈 조회) 로직을 실행
```

### Step 1: 티켓 정보 수집 (필수 — 이슈가 소스 오브 트루스)

> ℹ️ test-run 경유 시 이 Step은 스킵됩니다 (Gate에서 이미 이슈를 조회했으므로).
> 단독 호출 시에만 실행됩니다.

> **이슈(IMS) 티켓의 요구사항이 테스트시트의 기대결과를 결정한다.**
> 코드 분석은 "어떻게 구현되었는가"를 파악하는 데 사용하고,
> "어떻게 동작해야 하는가"는 반드시 이슈 티켓에서 가져온다.

```
1. 이슈 티켓 조회 (MCP 또는 Rest API)
   - issueIdOrKey: "{티켓}"
...
2. 요구사항 핵심 정리
   ...
   - 엣지 케이스: {이슈(IMS)에 언급된 특수 상황}

3. 이슈 접근 불가 시:
   - 사용자에게 이슈 내용 공유 요청
   - 또는 WebFetch로 이슈 페이지 접근 시도
...
```
✅ 올바른 흐름:
   이슈 요구사항 → 기대결과 도출 → 코드 분석으로 검증 방법 결정

❌ 잘못된 흐름:
   코드 분석 → 코드 동작을 기대결과로 설정 → 이슈 무시
   (이 경우 코드의 버그도 "정상"으로 판정하게 됨)
```

테스트시트에 이슈 요구사항 원문을 인용한다:

```markdown
### 1.3 핵심 변경 사항 (이슈 근거)

> **이슈 원문**: "{이슈 description에서 해당 부분 발췌}"
> **출처**: [{티켓}](https://developer.atlassian.net/browse/{티켓})
...
0. test/_shared/test_concerns.md — 반복 실수 방지 (가장 먼저!)
1. test/_shared/domain/{관련 도메인}.md — 테스트 대상 도메인 파일

[2단계] 템플릿 + 버전 규칙 (test/templates/):
5. test/templates/README.md — 파일 명명 규칙, 버전 결정 로직
6. test/templates/test_sheet_template.md — 테스트시트 표준 형식

[3단계] 시스템 아키텍처 (선택 사항 — test-gate에서 지정 시 로드):
7. {impact_references} — test-gate에서 선택한 디렉토리 혹은 파일들
   (예: ctx.service_metadata가 제공하는 services, dependencies, flow 정보 등)

[4단계] DB 스키마 (필수):
8. 도메인 관련 schema.prisma 혹은 .sql 파일
9. ctx.service_metadata가 제공하는 data contracts (존재 시)
```
...
ELSE (standalone 실행):
  → mkdir -p $CLAUDE_PROJECT_DIR/test/{티켓}/
  → ctx.ticket_folder = "test/{티켓}"
```
...
예) Waterfall 코드의 분기: - remain > batchPlanQty → 전량 할당 - remain < batchPlanQty → 부분 감소 ← TC 필요 - remain == 0 → 주문 제외 ← TC 필요 - Job 매핑 주문 < 전체 주문 → 필터링 효과 ← TC 필요
...
5. File generation: {ticket}_test_sheet_v{N}_{date}.md
...
1. API 엔드포인트 추적 (필수):
   ...
   - test/_shared/env/api_endpoints.md에서 서비스별 Base URL을 확인한다
...
━━━ BEFORE ━━━
  ...
  FROM external_service-out_order
  WHERE external_service-out_order_id = '{data.eo_id}'
  → state=REJECTED 확인

━━━ STIMULUS (Sequence) ━━━
  Step 1 [Node: OMS_API (HTTP)]:
    Method: POST
    Endpoint: https://order-api.example.com/order/event/cancel
    Body: { "orderId": "{data.oms_order_id}", "reason": "테스트" }
    Expected: HTTP 200
  
  Step 2 [Node: core-system_KAFKA (Event)]:
    Verify Message: service-out-cancel-topic 에 이벤트 발행 확인
...
> 실제 사례: `service-out_order.event_reason`은 리피킹 사유 컬럼이지만, 컬럼명만 보고 dead letter 지표로 오인하여 잘못된 검증 수행.
...
Step 3: 조건을 데이터 요구사항으로 변환
  ...
  → job_lifecycle_service-out_order_mapping.service-out_order_state = 'PICKING'
```
...
| 메타데이터                                  | 활용                                           | 예시                                                                     |
| ------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------ |
| `ctx.service_metadata` → `semanticContracts` | 컬럼의 비즈니스 의미, writer/reader, 상태 조건 | `service-out_order_state`는 비정규화 캐시 → 실시간 상태는 service-out에서 확인 |
| `ctx.service_metadata` → `dataFlowPaths`     | 엔티티 상태 라이프사이클, 상태 전이 순서       | containerLifecycle: 생성→피킹중→배치완료                                 |
| `ctx.service_metadata` → `nullSemantics`     | null/빈값의 비즈니스 의미                      | `worker_id = null` → 작업자 미배정 (PENDING 상태)                        |
| `ctx.service_metadata` → `db schemas`        | 상태 컬럼의 가능한 값, 컬럼 간 관계            | `container_state` enum 값 목록                                           |
| `ctx.service_metadata` → `api dependencies`  | API 호출 체인에서 상태 전이 포인트             | 피킹 시작 API → 주문 상태가 PICKING으로 변경                             |

```
참조 흐름:
  1. TC 기대값에 상태 관련 값이 있으면
  2. ctx.service_metadata의 semanticContracts에서 해당 컬럼 확인
...
| 사전 조건       | container에 복수 주문, overflow 상태                                             |
| 기대 결과       | Waterfall 분배 적용, quantity > 0                                                |
| **행위적 조건** | `service-out_order_state = 'PICKING'` (코드 근거: ContainerWebQueryApplication:668) |
```
...
| ③ Filter    | 조건부 필터링 (WHERE, 코드 분기)  | job_lifecycle_service-out_order_mapping 존재 여부      |
| ④ Outcome   | 최종 기대값 (①+②+③에서 도출)      | service-outOrderSkuGroups 배열 길이 = N                |
...
TT0000000042925
├─① batch 전체 주문: 8개
├─② Job 매핑 (job_lifecycle_service-out_order_mapping):
...
SELECT cl.container_code, cls.stock_management_unit_id, cls.quantity AS container_qty,
       bsms.service-out_order_id, bsms.quantity AS batch_plan_qty
...
ORDER BY cl.container_code, bsms.service-out_order_id
```
...
> ⚠️ behavioral_contracts 직접 생성은 test-gate.md 실행 결과가 없을 때만 허용되는 예외 케이스이다.
> Gate의 정규화 규칙(condition_id 유일성, schema)을 준수해야 한다.
  ...
  "tcs": {
    "TC-1": {
      ...
      "expected": {
        ...
        "fields": [
          {
            "path": "응답 JSON 경로 (예: service-outOrderSkuGroups[*].skus[*].quantity)",
            "source_db": "MCP 도구명",
            "source_table": "테이블명",
...
> 📐 formula 필드는 자유 텍스트 설명이며, 도출 트리 생성 시 다음 규칙을 따른다:
> - formula가 "A + B" 형식이면 2단계 트리 (root → A, B)
> - formula가 "A → B → C" 형식이면 선형 3단계 트리
> - formula가 자유 텍스트이면 단일 리프 노드로 표시
> - 자동 파싱 불가 시 formula 텍스트를 그대로 리프 노드에 표시
...
File generation: `{ctx.ticket_folder}/{ticket}_tc_spec.json`
...
| ----------- | ---------------------- | --------------------------- | --------------------- |
| ① 주문 생성 | POST /service-out/create  | service-out_order.state        | null → CREATED        |
...
| ④ 패킹 완료 | POST /packing/complete | service-out_order.state        | PICKING → PACKING     |
```
...
### ⭐ Step 4.7: Universal E2E Flow Visualization (필수)

> **원칙**: 복잡한 노드 간 연동을 포함하는 TC의 경우, 설계 단계에서 Mermaid 흐름도를 생성하여 시각화한다.
> 이는 `test-run` 실행 전 연동 구조의 정합성을 최종 확인하는 용도이다.

1. **흐름도 생성**:
   - `test/_shared/tools/flow_visualizer.py`를 호출하거나 수동으로 Mermaid Sequence Diagram 작성
   - 참여 노드(Participant)와 메시지 흐름(Arrow)을 명시

2. **테스트시트 삽입**:
   - 테스트시트의 `Section 3: 시나리오 목록` 하단에 `{ticket}_E2E_Flow.md` 또는 Mermaid 코드 블록 삽입

### Step 4.8: Test Baseline 기록 (필수)
...
| 서브섹션            | 내용                                             | 형식   |
| ------------------- | ------------------------------------------------ | ------ |
| 0.1 이슈(IMS) Digest     | 정규화된 요구사항 (ID, 요구사항, 출처)           | 테이블 |
| 0.2 Code Digest     | 구현 사항 (ID, 구현 내용, 변경 파일)             | 테이블 |
| 0.3 비교 결과       | 합집합 기준 일치/미기재/미구현                   | 리스트 |
| 0.4 영향 노드       | 노드명, 장치(Server/Browser/..), 프로토콜, 브랜치 | 테이블 |
| 0.5 테스트 유형     | api_test, db_verify, node_simulation 등 활성 여부 | 테이블 |
| 0.6 참조 체인       | 변경점 → 참조 경로 → 최종 소비자                 | 테이블 |
| 0.7 데이터 시나리오 | 분기 조합별 TC 매핑 + 누락 식별                  | 테이블 |
| 0.8 E2E Flow Map    | 노드 간 연동 맵 (Mermaid)                        | 그림   |

> `validate_test_sheet.py` Hook이 `### 0.1` ~ `### 0.7` 존재를 강제한다.
> Section 0.4~0.5는 Gate 1.8의 영향도 재검증에, 0.6~0.7은 TC 누락 방지에 사용된다.

### Step 4.6: SSOT 무결성 해시(Hash) 기록 (필수)

> **원칙**: 생성된 마크다운 테스트시트와 `tc_spec.json` 간의 동기화 상태를 보장하기 위해, 마크다운의 해시값을 JSON 내부에 기록합니다.
> 이 해시는 런타임(test-run) 시점에 수동 변경 여부를 검증하는 SSOT의 핵심 기준이 됩니다.

테스트시트 마크다운 파일과 `tc_spec.json` 작성이 완료되면 **반드시** 아래 파이썬 스크립트를 실행하여 해시값을 동기화합니다.

```bash
python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/ssot_hash_manager.py --mode update --markdown "{ctx.sheet}" --json "{ctx.tc_spec}"
```

> **주의**: 마크다운 파일이나 json 파일 생성을 모두 마친 **최종 단계**에서 실행해야 합니다.

---

## Output Format

> 출력 형식은 `test/templates/test_sheet_template.md` 기준으로 작성한다.

---

## Related Skills

- **test-gate**: Gate 출력(test_baseline)을 전달받아 사용 (이슈(IMS) 재조회 불필요)
