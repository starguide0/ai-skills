...
CLI 사용 예시:
  python3 behavioral_gate.py --mapping ARG-XXXXX_data_mapping.json --output ARG-XXXXX_behavioral_gate.json
"""
import argparse
import glob
import json
...
from hook_utils import resolve_content, TC_ID_PATTERN, _tc_sort_key
...
def find_mapping_file(partial_results_dir: str) -> str | None:
    """partial_results 디렉터리의 부모에서 *_data_mapping.json 파일을 찾는다."""
    parent = os.path.dirname(os.path.abspath(partial_results_dir))
    candidates = glob.glob(os.path.join(parent, "*_data_mapping.json"))
    if candidates:
        return candidates[0]
    return None
...
def check_behavioral_gate_for_tc(tc_id: str, mapping_data: dict) -> tuple[bool, str]:
    ...
    # N/A: 검증 대상 아님 → 경고만 출력하고 통과
    if verdict == "N/A":
        sys.stderr.write(
            f"Warning: {tc_id} behavioral_check.verdict=N/A — 검증 범위 외 TC로 처리하여 통과합니다.\n"
        )
        return True, ""

    # PENDING: 검증 미완료 → BLOCKED (사유 명시)
    if verdict == "PENDING":
        return False, (
            f"{tc_id}: behavioral_check.verdict=PENDING — 검증 미완료. "
            "데이터 매핑의 behavioral_check를 완료한 후 다시 시도하세요."
        )

    # 그 외 verdict가 PASS가 아닌 경우 → deny
...
def run_cli_mode(mapping_path: str, output_path: str):
    ...
    for tc_id, tc_entry in sorted(mappings.items(), key=lambda x: _tc_sort_key(x[0])):
        ...
        if verdict == "PASS":
            ...
            # N/A: 검증 범위 외 TC → 경고와 함께 PASS 처리
            results.append({
                "tc_id": tc_id,
                "gate": "PASS",
                "reason": "behavioral_check.verdict=N/A (검증 범위 외, 경고: 의도적 N/A인지 확인 필요)",
            })
            pass_count += 1
            ...
            results.append({
                "tc_id": tc_id,
                "gate": "BLOCKED",
                "reason": "behavioral_check.verdict=PENDING — 검증 미완료",
            })
        else:
            conditions = behavioral_check.get("conditions", [])
            conditions_str = format_conditions(conditions)
...
def main():
    ...
    if args.mapping:
        ...
        if not output_path:
            ...
            # *_data_mapping → *_behavioral_gate
            if base.endswith("_data_mapping"):
                base = base[: -len("_data_mapping")]
