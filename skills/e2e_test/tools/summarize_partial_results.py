...
from hook_utils import TC_ID_PATTERN, _tc_sort_key  # noqa: F401
...
def load_tc_files(directory):
    ...
    for path in sorted(dir_path.glob("*.json")):
        ...
        try:
            ...
            sys.stderr.write(f"Warning: Failed to load/parse {path.name}: {e}\n")
            continue
    return tc_results
...
def summarize(tc_results):
    ...
    stats = {"total": 0, "pass": 0, "fail": 0, "nt": 0, "blocked": 0, "incomplete": 0, "skipped": 0, "partial_mapped": 0}
    tcs = []
    failed_tcs = []
    blocked_tcs = []
    checks = {}
    ...
    for stem, data in tc_results:
        ...
        stats["total"] += 1
        status_key = {"PASS": "pass", "FAIL": "fail", "N/T": "nt", "NT": "nt",
                      "BLOCKED": "blocked", "INCOMPLETE": "incomplete",
                      "SKIPPED": "skipped", "PARTIAL_MAPPED": "partial_mapped"}.get(status, "incomplete")
    ...
    failed_tcs.sort(key=_tc_sort_key)
    blocked_tcs.sort(key=_tc_sort_key)
...
def main():
    ...
    s = summary["stats"]
    # dict 내부 키는 "nt"를 사용하되, 표시 레이블은 "N/T"로 명시적으로 매핑
    _label = {"nt": "N/T", "partial_mapped": "PARTIAL_MAPPED"}
    print(
        f"✅ Summary: {s['total']} TCs — "
        f"PASS={s['pass']}, FAIL={s['fail']}, {_label['nt']}={s['nt']}, "
        f"BLOCKED={s['blocked']}, INCOMPLETE={s['incomplete']}, "
        f"{_label['partial_mapped']}={s.get('partial_mapped', 0)} ({s['pass_rate']})"
    )
