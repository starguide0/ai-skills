...
Usage:
  ...
  python3 stimulus_executor.py \\
    --method POST \\
    --url "https://order-api.example.com/order/event/cancel" \\
  ...
  python3 stimulus_executor.py \\
    --auth-login \\
    --auth-url "https://auth-api.example.com/api/auth/login" \\
...
try:
    ...
    print("설치: pip3 install -r $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/requirements.txt", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    parser.add_argument("--step", help="실행 스텝 식별자 (출력 JSON에 포함)")
    parser.add_argument("--output", help="출력 파일 경로 (미지정 시 stdout)")
    parser.add_argument("--timeout", type=int, default=30, help="요청 타임아웃 (초)")
    parser.add_argument("--no-timestamp", action="store_true", help="결과 JSON에서 timestamp 필드 제외 (비결정성 제거)")
    ...
    if args.auth_login or args.auth_url:
        ...
        if args.auth_body:
            try:
                ...
                sys.stderr.write(f"Error: Invalid JSON format for authentication body: {e}\n")
                sys.exit(1)
        else:
    ...
    if args.body:
        try:
            ...
            sys.stderr.write(f"Error: Invalid JSON format for request body: {e}\n")
            sys.exit(1)
    elif args.body_file:
        try:
            ...
            sys.stderr.write(f"Error: Invalid JSON in body-file {args.body_file}: {e}\n")
            sys.exit(1)
