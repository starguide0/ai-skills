...
def get_nested(obj, path: str):
    ...
    e.g. "response.body.service-outOrderSkuGroups"
...
def collect_all_quantities(body) -> list:
    ...
    Generic fallback: recursive search for any 'quantity' field in the response.
    """
    quantities = []
    ...
    def _collect(obj):
        if isinstance(obj, dict):
            if "quantity" in obj:
                quantities.append(obj["quantity"])
            for v in obj.values():
                _collect(v)
...
def check_all_field(body, field_name: str, expected_value):
    ...
    field_name can be dot-navigated (e.g. service-outOrderState.name).
    ...
    # e.g. field_name="service-outOrderState" → look for arrays containing dicts with that field
    for key, val in body.items():
    ...
    for key, exp_val in expected.items():
        ...
        if key == "total_qty":
            ...
            checks.append({
                ...
                "actual": total,
                "result": result,
                "detail": f"sum(service-outOrderSkuGroups[*].skus[*].quantity)={total}",
            })
            continue
...
def main():
    ...
    parser.add_argument(
        "--expected-file",
        required=False,
        help="Path to the expected API response JSON file",
    )
    parser.add_argument(
        "--actual-file",
        required=True,
    ...
    if not args.expected and not args.expected_file:
        print("ERROR: Either --expected or --expected-file must be provided", file=sys.stderr)
        sys.exit(1)

    # Parse expected JSON
    expected = None
    if args.expected_file:
        if not os.path.exists(args.expected_file):
            print(f"ERROR: expected-file not found: {args.expected_file}", file=sys.stderr)
            sys.exit(1)
        try:
            with open(args.expected_file, "r", encoding="utf-8") as f:
                expected = json.load(f)
        except json.JSONDecodeError as e:
            print(f"ERROR: expected-file is not valid JSON: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        try:
    ...
    if not isinstance(expected, dict):
        print("ERROR: Expected data must be a JSON object (dict)", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output:
        ...
        print(f"Verdict saved: {args.output}", file=sys.stderr)
    else:
        print(output_str)
