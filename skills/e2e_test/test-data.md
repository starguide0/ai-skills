---
name: test-data
description: "Maps test data to test cases using Discovery, Structural Signatures, and Smart Recycling."
...
- **Discovery**: DB 기반 자동 검색
- **Structural Signature Mapping**: 샘플 파일(Excel 등)의 구조적 명세를 파일별 버전으로 관리하고 TC와 명시적으로 매핑
- **Smart Recycling**: `samples/` 폴더 내의 기존 데이터를 구조 시그니처 판정을 통해 지능적으로 재사용
...
- `test_concerns.md` — DB 스키마 오류, 반복 실수 방지 (가장 먼저 로드)
- `env/url.md` — 환경별 서비스 URL
- `env/api_endpoints.md` — 서비스별 API 목록 (API 기반 데이터 탐색에 사용)
- `rule/_caution_mcp_usage.md` — MCP PostgreSQL 도구 안전 사용 가이드
- `rule/_caution_missing_tables.json` — DB 스키마에 누락된 테이블 목록
- `$CLAUDE_PLUGIN_ROOT/skills/e2e_test/rules/_caution_common_errors.md` — 반복 발생 오류 패턴
...
| 필드                   | 출처      | 필수   | 설명                                                                                 |
| ---------------------- | --------- | ------ | ------------------------------------------------------------------------------------ |
| Test Sheet             | test-plan | Y      | {ticket}_test_sheet_v{N}_{date}.md — Includes per-TC data requirements                      |
| 행위적 조건            | test-plan | N      | TC별 행위적 조건 (테스트시트 내 포함). 없으면 CoT Fallback으로 자체 추론             |
| {ticket}_tc_spec.json | test-plan | Conditional | Structured JSON for behavioral conditions & data reqs — Fallback to Markdown mode if missing |
| ctx.tc_spec            | test-plan | Conditional | Absolute path to tc_spec.json. Restore via Glob if missing                                   |
| ctx.sheet_version      | test-plan | Y           | v{N} — Major version (Used for skip determination)                                           |

### OUTPUT

| 필드                             | 소비자                            | 설명                                                     |
| -------------------------------- | --------------------------------- | -------------------------------------------------------- |
| data_mapping.json                | test-scheduler, test-provisioning | {ticket}_data_mapping.json — Per-TC mapping results      |
| ctx.data_mapping                 | test-scheduler, test-run          | Absolute path to data_mapping.json                       |
...
0. test/_shared/test_concerns.md — DB 스키마 오류, 반복 실수 방지 (가장 먼저!)
1. test/_shared/env/url.md — 환경별 서비스 URL
2. test/_shared/env/api_endpoints.md — 서비스별 API 목록

[2단계] 시스템 아키텍처:
3. ctx.service_metadata가 제공하는 DB schema 정보 — 관련 서비스의 테이블/컬럼 구조

[3단계] DB 오류 방지 (rules/):
4. test/_shared/rule/_caution_missing_tables.json — DB 테이블 정보 (누락분)
5. $CLAUDE_PLUGIN_ROOT/skills/e2e_test/rules/_caution_common_errors.md — DB 쿼리 오류 패턴
...
```
ctx 복원: Glob(`{ticket_folder}/{ticket}_gate_*.json`) → 타임스탬프 내림차순 정렬 → 첫 번째 파일 사용

IF ctx.tc_spec 없으면:
  → Glob: {ctx.ticket_folder}/{ticket}_tc_spec.json  # ← Includes ticket_folder (CWD independent)
...
`ctx.service_metadata`가 제공하는 data contracts를 참조하여 검증 보강:
...
> - summary 정합성: mapped + not_found + provisioning_needed + provisioned + skipped + behavioral_mismatch + capture_planned + blocked == total_tcs
>   (behavioral_mismatch: mappings의 status는 NOT_FOUND로 전환되지만, summary에서는 behavioral_mismatch 카운터로 별도 집계 — not_found와 이중 계산하지 않음)

> ⚠️ BEHAVIORAL_MISMATCH는 내부 분류용으로만 사용되며, 합산에는 NOT_FOUND로 계산된다.
> 즉, behavioral_mismatch 카운터는 DEBUG 정보로만 보고하고 summary 합산에서는 제외한다.
...
```
File: {ctx.ticket_folder}/{ticket}_data_mapping.json
...
> Write data_mapping.json  # completed_at 포함하여 단일 Write
> ```
>
> ※ 두 번 Write 금지: 첫 Write 후 중단 시 completed_at: null 영구 고착 → test-run Step 4 캐시 무효화 실패
>
> **Provisioning 후 completed_at 재갱신**:
> test-provisioning Step 4.5에서 data_mapping.json을 업데이트할 때, completed_at을 now()로 재갱신해야 한다.
...
생성된 파일:
- test/PROJ-123_feature_name/PROJ-123_data_mapping.json
```
...
3. [승인 시] -> `test-provisioning` 스킬 호출
   - Input: {ticket}_data_mapping.json, Target TC
   - 출력: 업데이트된 data_mapping.json
```
...
- `{ctx.ticket_folder}/{ticket}_data_mapping.json`
...
| PARTIAL_MAPPED      | 테이블은 찾았으나 필드 일부만 매핑 가능한 경우                          |
...
| BLOCKED             | 행위적 조건 미충족 등으로 실행이 차단된 TC                              |

### Summary 필드 설명

- **total_tcs**: 전체 테스트 케이스 수
- **mapped**: 기존 데이터로 매핑 성공한 수
- **not_found**: 데이터를 찾지 못한 수 (Manual 확인 필요)
- **provisioning_needed**: 신규 생성(Provisioning)이 필요한 수
- **provisioned**: 신규 생성(Provisioning) 완료된 수
- **blocked**: 행위적 조건 미충족 등으로 실행이 차단된 수
- **skipped**: 의도적으로 제외된 수
- **capture_planned**: 스크린샷 캡처가 예정된 수
- **behavioral_mismatch**: 구조적 매칭 성공했으나 행위적 조건 미충족한 TC 수 (내부적으로 NOT_FOUND로 전환됨)
- **coverage**: 매핑된 TC의 비율
...
- **test-plan**: 테스트 계획 생성 (이슈 요구사항 및 데이터 요구사항 포함)
