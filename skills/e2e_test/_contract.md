...
| **문서** | `~/project/skills/docs/e2e_test/*.md` | ⛔ 읽기 전용 | 아키텍처 설명서. 코드 변경 후 별도 동기화 |
...
|--------|---------|--------|---------|------|
| `stimulus_executor.py` | `"response"` | `summarize_partial_results.py` | `"api_response"` (우선) / `"response"` (fallback) | ✅ `checks[tc_id]` 저장 시 fallback 있음 (`summarize_partial_results.py:119`) |

> 📌 stimulus_executor 출력 표준 키: `"response"` (api_response는 하위 호환용 fallback 키)
> summarize_partial_results.py는 `api_response` 우선 → `response` fallback 순서로 읽는다.
> 신규 구현은 `"response"` 키를 사용한다.
| `validate_test_sheet.py` | TC ID `[A-Za-z0-9.\-]+` | `summarize_partial_results.py` | natural sort | ✅ 일치 |
| `behavioral_gate.py` | `results[].gate` | `test-scheduler.md` Step 0.5 | `result.gate` | ✅ `is_tc_result_file()` 정규식 `[A-Za-z0-9.\-]+`는 점(.) 포함 TC ID 정상 지원 |

> ACTIVE 마커는 **TC 목록 테이블의 상태 컬럼**에서 탐지한다 (별도 마커 테이블 없음).
> 탐지 패턴: `| TC-xxx | ... | ACTIVE |` (테이블 행의 마지막 컬럼)
| `summarize_partial_results.py` | `{pass, fail, nt, blocked, incomplete, skipped}` | `generate_mermaid_diagrams.py` | 동일 key | ✅ 일치 |
...
| `~/project/skills/docs/e2e_test/HOOKS.md` 예시 JSON | 문서 동기화 (읽기 전용이므로 마지막에 별도 업데이트) |
...
- [ ] Hook exit code 변경 → `~/project/skills/docs/e2e_test/HOOKS.md` 규칙과 일치하는지 확인 (deny = exit(0), 에러 = exit(1))
