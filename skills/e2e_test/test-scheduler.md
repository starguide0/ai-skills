...
| ctx.data_mapping | test-data | Y | data_mapping.json 절대 경로 — TC별 매핑 상태 |
| 행위적 조건 (in data_mapping) | test-data | N | data_mapping.json 내 TC별 behavioral_check — Pre-flight 검증에 사용 |
...
| 필드 | 소비자 | 설명 |
|------|--------|------|
| test_execution_plan.json (DAG) | test-run (Step 5.1, 5.2) | 실행 계획 파일 ({ctx.ticket_folder}/{ticket}_execution_plan.json) |
...
| ctx 필드 | 복원 소스 | 복원 방법 |
|----------|-----------|-----------|
| sheet | `{ctx.ticket_folder}/{ticket}_test_sheet_v*.md` | Glob → 최고 버전 선택 |
