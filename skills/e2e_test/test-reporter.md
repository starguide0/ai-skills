---
name: test-reporter
description: "Aggregates partial test results from multiple batches and generates the final Test Report (Markdown)."
---
...
- (없음 — 결과 파일만 읽는다.)
- `rules/_report_format_rules.md` → test-run Step 6 진입 시 Main 오케스트레이터가 로드. 본 subagent는 ctx에서 상속받아 사용.
...
| ctx.sheet           | test-plan         | Y      | Test sheet file path ({ticket}_test_sheet_v{N}_{date}.md)                                     |
| ctx.test_baseline   | test-gate         | Y      | Gate 통과 시 생성된 기준 객체 (issue_digest, code_digest, test_types 등)                        |
...
| 필드                       | 소비자            | 설명                                                      |
| -------------------------- | ----------------- | --------------------------------------------------------- |
| Test Result Report (Test Report) | User            | `{ticket}_report_test_result.md` and related assets stored in `{ctx.ticket_folder}/reports/{date}_v{N}.{M}/` |
| ctx.report_file            | test-post         | 생성된 보고서 파일 절대 경로 (`{ticket}_report_test_result_v{N}.{M}_{date}.md`) |
| ctx.result_file            | test-post         | TC별 결과 요약 파일 절대 경로 (`{ticket}_test_result_v{N}.{M}_{date}.md`)       |
| template_compliance_check  | test-run (검증용) | 필수 섹션 포함 여부 체크 결과                             |

### INTERNAL (다른 스킬이 몰라도 되는 것)

- **보고서 디렉토리 격리**: `{ctx.ticket_folder}/reports/{date}_v{N}.{M}/` 전용 폴더 자동 생성 로직
- **자산 마이그레이션**: 임시 `screenshots/` 폴더의 이미지를 보고서 하위의 `assets/`로 이동 또는 Base64 임베딩

- 보고서 마크다운 출력 규칙 (\_report_format_rules.md 기반)
...
> - `ctx.sheet` 없으면 → Glob `{ctx.ticket_folder}/{ticket}_test_sheet_v*.md` → 최고 버전 선택
...
1.  **요약 생성 (Python)**: partial_results/ 전체를 단일 요약 JSON으로 집계
    ...
    ```bash
    python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/summarize_partial_results.py \
...
2.  **Mermaid 초안 생성 (Python)**: \_summary.json에서 다이어그램 골격 생성
    ...
    > \_report_format_rules.md의 "Step 5 후반 사전 생성" 지침과 다름 — reporter Step 2가 권위 있는 생성 시점.

    ```bash
    python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/generate_mermaid_diagrams.py \
      --summary {ctx.ticket_folder}/partial_results/_summary.json \
      --output {ctx.ticket_folder}/partial_results/_mermaid_drafts.json
    ```

    → pie chart(완성), sequence/state/before_after(골격) 생성.

    **Step 2-b: URL 생성 (다이어그램 생성 직후)**

    ```bash
    python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/generate_mermaid_urls.py \
      --ticket {ticket} --output {ctx.ticket_folder}/.mermaid_urls_{ticket}.json
    ```

    → 생성된 다이어그램 각각의 렌더링 URL을 JSON으로 저장 (문서 삽입용)

3.  **LLM 검토/보강**: `_summary.json` + `_mermaid_drafts.json` 2개 Read + (있으면) `{ticket}_tc_spec.json` Read - 생성된 Mermaid 골격을 검토하고 비즈니스 컨텍스트에 맞게 Note/이벤트/흐름을 미세 조정 (바닥부터 새로 작성하지 말고 드래프트 수정을 우선) - 마크다운 문서 편집 시 Mermaid 코드는 반드시 ` ```mermaid ` 코드블록 사용 (일반 ` ``` ` 절대 금지) - TC별 자연어 근거, 의미 해석, FAIL 원인 분석

    > 도출 트리 생성 규칙: test-plan.md의 formula 필드 파싱 규칙을 따른다.
    > formula가 자동 파싱 불가능한 경우, 텍스트를 그대로 표시하고 "[수동 확인 필요]" 태그 추가.
...
④ Outcome:   기대값 = {computed_value} (실행 시 동적 계산)` # static_fields (고정값 필드) 처리: # static_fields는 full 4단계 트리 대신 1줄 형식 사용: # "④ 기대결과: {value} (고정값 — 도출 불필요)" # 예: "④ 기대결과: 200 (HTTP Status — 고정값)" # fields[]에 있는 formula 필드만 4단계 트리로 출력한다.
...
5.  **문서 생성**: `_report_format_rules.md` 기반으로 마크다운 생성

6.  **구조 검증 (Python)**: 생성된 보고서 문서의 필수 섹션 검사

    ```bash
    python3 $CLAUDE_PLUGIN_ROOT/skills/e2e_test/tools/validate_report_structure.py \
      --file {ctx.ticket_folder}/{ticket}_report_test_result_v{N}.{M}_{YYYY-MM-DD}.md \
      --schema $CLAUDE_PLUGIN_ROOT/skills/e2e_test/rules/_wiki_output_rules.json \
      --output {ctx.ticket_folder}/partial_results/_validation.json
    ```

    → FAIL 항목만 LLM이 수정 (설정된 schema 중 PASS는 무시)
    ...
    >
    > - `"status": "FAIL"` 항목 → 해당 섹션을 보고서에서 직접 수정
    > - `"status": "PASS"` 항목 → 무시
...
- `ctx.test_baseline.issue_digest` — 이슈(IMS) 요구사항 (수정 이유)
...
| UI 캡쳐           | screenshots[]                | `![TC번호](assets/...)` 이미지 삽입 (보고서 하위 assets/ 경로) |
...
| **_report_format_rules.md** | 문서 구조 + Mermaid 변환 + 체크리스트          |

- **시각적 증거 임베딩 (Visual Evidence)**:
  - `assets/` 폴더 내의 모든 시각적 자산은 보고서에 임베딩되어야 합니다.
  - **이미지**: `![캡처 설명](assets/filename.png)` 형식을 사용합니다.
  - **동영상 (WebP)**: `![작업 과정 레코딩](assets/filename.webp)` (플랫폼 지원 시 자동 렌더링) 또는 `[동영상 확인](assets/filename.webp)` 링크 형식을 사용합니다.
  - 경로 주의: 보고서 마크다운 파일 기준으로 상대 경로인 `assets/` 하위를 정확히 참조하십시오.
